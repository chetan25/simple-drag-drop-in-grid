...
import styles from "../styles/flex-grid.module.css";
...
const Slot = (props: any) => {
  const customStyles = {
    ...
    ...props.box.styles,
    gridColumnStart: props.box.gridColumnStart,
    gridColumnEnd: props.box.gridColumnEnd,
  };
...
const Groups = () => {
  ...
  const handleDragStart = (data: any) => (event: any) => {
    ...
    // if the handle bar is the one thats being dragged proceed else not
  ...
  const handleDragOver = (data: any) => (event: any) => {
    ...
    // console.log(dropElGroupId, dragElGroupId);
  ...
  const onDragLeave = (data: any) => (event: any) => {
    console.log("leave");
    event.target!.setAttribute("data-draggingOver", "false");
    if (event.target.className.includes("dropzone")) {
      event.target.style.borderTop = "";
      event.target.style.borderBottom = "";
    }
  ...
  const handleDrop = (data: any) => (event: any) => {
    ...
    if (dragElGroupId !== dropElGroupId) {
      ...
      const colStart = draggedEl.gridColumnStart;
      const colEnd = draggedEl.gridColumnEnd;
      ...
      //   console.log(dropIndex, dragIndex, "dragIndex");
    ...
    return state.slots.map((slot, index) => (
      <div
        ...
        style={{
          ...
          gridTemplateRows: slot.gridTemplateRows,
  ...
  return (
    <div className={styles.container}>
      <main className={styles.main}>
        <h2>
          Simple Grid Based Approach - Grid constrained to One Direction only
          (columns)
        </h2>
        <Groups />
      </main>
